...
```{r}
df <- read_sav(here("data/qualtrics/4_demographics", "12pp_demo.sav")) %>%
...
```{r}
#| echo: false
ltm::cronbach.alpha(data = select(df, IMS1.12pp:IMS22.12pp), na.rm = T)
```
...
```{r}
write_sav(df, here("data/qualtrics/5_final", "12pp_complete.sav"))
write_sav(df_sum, here("data/qualtrics/5_final", "12pp_summary.sav"))
```
:::
