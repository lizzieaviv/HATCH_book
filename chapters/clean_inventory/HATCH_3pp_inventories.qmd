...
```{r}
df <- read_sav(here("data/4_demographics", "3pp_demo.sav")) %>%
  mutate(across(where(is.character), ~ na_if(.x, ""))) # replace blank cells with NA

df.old <- read_sav(here("data/3_renamed", "3pp_renamed.sav")) %>%
  mutate(across(where(is.character), ~ na_if(.x, ""))) # replace blank cells with NA
```
:::
...
```{r}
setnames(df,
  old = c("MPAS7.3pp", "MPAS8.3pp", "MPAS9.3pp", "MPAS10.3pp", 
          "MPAS11.3pp", "MPAS12.3pp", "MPAS13.3pp", "MPAS14.3pp"),
  new = gsub(".3pp", "_r.3pp", 
             c("MPAS7.3pp", "MPAS8.3pp", "MPAS9.3pp", "MPAS10.3pp",
               "MPAS11.3pp", "MPAS12.3pp", "MPAS13.3pp", "MPAS14.3pp"))
)
```
...
df <- df %>%
  ...
  mutate(MPAS_attach.3pp = sum(c(c_across(MPAS3.3pp:MPAS7_r.3pp),
                                 MPAS10_r.3pp, MPAS14_r.3pp, 
                                 MPAS18.3pp, MPAS19.3pp), na.rm = F),
         MPAS_host.3pp = sum(c(MPAS1.3pp, MPAS2.3pp,
                               c_across(MPAS15.3pp:MPAS17.3pp)), na.rm = F),
         MPAS_pleasure.3pp = sum(c(MPAS8_r.3pp, MPAS9_r.3pp,
                                   c_across(MPAS11_r.3pp:MPAS13_r.3pp)), na.rm = F)) %>%
...
df <- df %>%
  ...
  mutate(MPAS_attach.3pp = labelled(MPAS_attach.3pp, label = "MPAS: Quality of Attachment"),
         MPAS_host.3pp = labelled(MPAS_host.3pp, label = "MPAS: Absence of Hostility"),
         MPAS_pleasure.3pp = labelled(MPAS_pleasure.3pp, label = "MPAS: Pleasure in Interaction"))
```
...
```{r}
#| echo: false
check_continuous(df, MPAS_attach.3pp:MPAS_pleasure.3pp) %>% knitr::kable()
```
...
```{r}
#| echo: false
ltm::cronbach.alpha(select(df, MPAS3.3pp:MPAS7_r.3pp, MPAS10_r.3pp, MPAS14_r.3pp, MPAS18.3pp, MPAS19.3pp), na.rm = T)
```

####### Absence of Hostility
```{r}
#| echo: false
ltm::cronbach.alpha(select(df, MPAS1.3pp, MPAS2.3pp, MPAS15.3pp:MPAS17.3pp), na.rm = T)
```

####### Pleasure in Interaction
```{r}
#| echo: false
ltm::cronbach.alpha(select(df, MPAS8_r.3pp, MPAS9_r.3pp, MPAS11_r.3pp:MPAS13_r.3pp), na.rm = T)
```
...
df <- df %>%
  ...
  mutate(across(c(PPAS13.3pp, PPAS16.3pp),
                ~ case_when(. == 1 ~ 5, . == 2 ~ 3, . == 3 ~ 1,
                            .default = NA))) %>%
  mutate(PPAS8.3pp = case_when(PPAS8.3pp == 1 ~ 5,
                               PPAS8.3pp == 2 ~ 1, .default = NA))
```
...
setnames(df,
  old = c("PPAS4.3pp", "PPAS5.3pp", "PPAS7.3pp", "PPAS8.3pp", "PPAS9.3pp",
          "PPAS10.3pp", "PPAS11.3pp", "PPAS12.3pp", "PPAS13.3pp", "PPAS14.3pp",
          "PPAS15.3pp", "PPAS16.3pp"),
  new = gsub(".3pp", "_r.3pp",
             c("PPAS4.3pp", "PPAS5.3pp", "PPAS7.3pp", "PPAS8.3pp", "PPAS9.3pp",
               "PPAS10.3pp", "PPAS11.3pp", "PPAS12.3pp", "PPAS13.3pp", "PPAS14.3pp",
               "PPAS15.3pp", "PPAS16.3pp"))
)
```
...
df <- df %>%
  ...
  mutate(PPAS_patience.3pp = sum(c(PPAS1.3pp, PPAS2.3pp, PPAS6.3pp, PPAS11_r.3pp, PPAS13_r.3pp,
         ...
         PPAS_pleasure.3pp = sum(c(PPAS4_r.3pp, PPAS5_r.3pp, PPAS8_r.3pp, PPAS9_r.3pp, PPAS10_r.3pp,
                                   PPAS12_r.3pp, PPAS15_r.3pp), na.rm = F),
...
df <- df %>%
  ...
  mutate(PPAS_patience.3pp = labelled(PPAS_patience.3pp, label = "PPAS: Patience and Tolerance"),
         PPAS_pleasure.3pp = labelled(PPAS_pleasure.3pp, label = "PPAS: Pleasure in Interaction"),
         PPAS_affection.3pp = labelled(PPAS_affection.3pp, label = "PPAS: Affection and Pride"))
```
...
```{r}
#| echo: false
check_continuous(df, PPAS_patience.3pp:PPAS_affection.3pp) %>% knitr::kable()
```
...
```{r}
#| echo: false
ltm::cronbach.alpha(select(df, PPAS1.3pp, PPAS2.3pp, PPAS6.3pp, PPAS11_r.3pp, PPAS13_r.3pp,
                           PPAS17.3pp, PPAS18.3pp, PPAS19.3pp), na.rm = T)
```
####### Pleasure in Interaction
```{r}
#| echo: false
ltm::cronbach.alpha(select(df, PPAS4_r.3pp, PPAS5_r.3pp, PPAS8_r.3pp, PPAS9_r.3pp, PPAS10_r.3pp,
                           PPAS12_r.3pp, PPAS15_r.3pp), na.rm = T)
```

####### Affection and Pride
```{r}
#| echo: false
ltm::cronbach.alpha(select(df, PPAS3.3pp, PPAS7_r.3pp, PPAS14_r.3pp, PPAS16_r.3pp), na.rm = T)
```
...
check_categorical(df, PSQI5a.3pp:PSQI8.3pp) %>% knitr::kable()
...
```{r}
#| echo: false
print_slice(df, PSQI9.3pp, PSQI_qual.3pp)
```
...
```{r}
#| echo: false
print_slice(df, PSQI2.3pp, PSQI5a.3pp, PSQI_lat.3pp)
```
...
```{r}
#| echo: false
print_slice(df, PSQI4.3pp, PSQI_dur.3pp)
```
...
```{r}
#| echo: false
print_slice(df, PSQI1_raw.3pp, PSQI3_raw.3pp, PSQI4.3pp, PSQI_effic.3pp)
```
...
```{r}
#| echo: false
print_slice(df, PSQI5b.3pp:PSQI5j.3pp, PSQI_disturb.3pp)
```
...
```{r}
#| echo: false
print_slice(df, PSQI6.3pp, PSQI_med.3pp)
```
...
```{r}
#| echo: false
print_slice(df, PSQI7.3pp, PSQI8.3pp, PSQI_daydys.3pp)
```
...
```{r}
#| echo: false
print_slice(df, PSQI_qual.3pp:PSQI_global.3pp)
```
:::
:::::
:::::::::
...
```{r}
write_sav(df, here("data/5_complete", "3pp_complete.sav"))
write_sav(df_sum, here("data/6_summary", "3pp_summary.sav"))
```
:::
:::
