...
```{r}
df <- read_sav(here("data/qualtrics/4_demographics", "0pn_demo.sav")) 
...
```{r}
#| echo: false
ltm::cronbach.alpha(data = select(df, IRI1.pn:IRI28.pn), 
                    CI = T, 
                    na.rm = T)
```
...
```{r}
#| echo: false
ltm::cronbach.alpha(data = select(df, contains("LAPR") & !contains("_")), 
                    CI = T, 
                    na.rm = T)
```
...
```{r}
#| echo: false
check_continuous(df, c(MAAS_tot.pn:MAAS_qual.pn, PAAS_tot.pn:PAAS_qual.pn)) %>% knitr::kable()
```
...
```{r}
#| echo: false
inspect_labels(df, contains("MAAS") | contains("PAAS")) 
```
:::
...
```{r}
write_sav(df, here("data/qualtrics/5_final", "0pn_complete.sav"))
write_sav(df_sum, here("data/qualtrics/5_final", "0pn_summary.sav"))
```
:::
